...
- [ ] 階段十：實際跑一次指南第七步的「路徑與用詞健康檢查」，全面盤點 `2ndbrain`。這個檢查寫進指南後從沒真的跑過，2026-08-06 靠人工翻才發現 vault `AGENTS.md` 的〈三層結構〉是反向舊版——正是它該抓到的東西
...
├── AGENTS.md              # 本檔：專案藍圖
├── CLAUDE.md              # 橋接檔：@AGENTS.md，供有 CLAUDE.md 時只讀 CLAUDE.md 的 Claude Code 使用
├── handoff.md             # 交接檔
...
- **資料夾結構同時寫在三個地方，改一處要當場查另外兩處**：指南（多張表＋兩段腳本）、`2ndbrain/AGENTS.md`（〈三層結構〉**和**〈資料夾規則〉表，同一份檔案內就有兩處）、`README.md`。2026-08-06 發現 vault 那份的〈三層結構〉還是反向舊版，與同檔表格自相矛盾——改名時只改了有編號的地方，清單順序漏掉
...
- **家目錄底下的檔案狀態要標機器名，而且每台都要各自驗。** `~/.claude/`、`~/.codex/`、`~/.config/opencode/`、`~/.gemini/` 不在雲端硬碟裡，不會跨電腦同步；只有本 repo 與 `2ndbrain` 會同步。交接寫「四份全域規則檔到齊」而沒標是哪台，下一台接手會誤判成已完成——2026-08-06 在 `NB-YI` 實測時四份只有一份（`PC-YI-FY` 那筆紀錄本身沒錯，錯在讀起來像專案狀態）。
